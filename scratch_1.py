...
class Bullet(Widget):
    ...
    def move(self):
        self.time += 1 / 61  # Increment time based on the frame rate
...
class Bombshell(Widget):
    ...
    def move(self):
        self.time += 1 / 30  # Increment time based on the frame rate
        ...
        # Remove the bombshell if it goes off-screen or reaches the ground
        if self.y < 0 or self.y > Window.height or self.x < 0 or self.x > Window.width:
...
class Laser(Widget):
    def __init__(self, pos, angle, velocity, **kwargs):
        ...
        self.pos = pos
        self.angle = angle
        self.velocity = velocity  # constant velocity
        self.dx = velocity * math.cos(math.radians(angle))
        self.dy = velocity * math.sin(math.radians(angle))
        self.time = 0
        with self.canvas:
...
class CannonApp(App):
    ...
    def update_projectyles(self, dt):
        ...
        for bombshell in self.bombshells:
            bombshell.move()
            self.check_collisions(bombshell)

        for laser in self.lasers:
            laser.move()
            self.check_collisions(laser)
    ...
    def load_profiles(self):
        ...
        try:
            ...
            for line in lines:
                ...
                load_button = Button(
                    ...
                    on_release=lambda btn: self.init_game(name, int(levels_completed), int(score))
                )
        ...
        self.root.get_screen('loadgame').add_widget(scroll_view)
